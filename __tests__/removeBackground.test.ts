...
async function main() {
  ...
  // ── O. Expired entitlement — quota gate re-applies ────────────────────────
  console.log('\nO. expired premium entitlement — quota gate re-applies:');

  // O1. Expired premium + exhausted quota → 403 (Photoroom never called)
  {
    const overrides = (require('../server/remove-background') as any)._testOverrides as {
      skipAuth: boolean;
      testUserId: string;
    ...
    overrides.skipAuth = true;
    overrides.testUserId = 'user-expired-entitlement-exhausted';
    overrides.mockPremium = true;
    overrides.mockPremiumExpiresAt = new Date(Date.now() - 1_000).toISOString();
    overrides.mockQuota = { allowed: false, count: 20, remaining: 0 };
    ...
    assertEq(res._status, 403,
      'O1: expired premium + exhausted quota → HTTP 403 (not 200)');
    assertEq((res._body as any)?.error, BG_REMOVAL_LIMIT_REACHED,
      `O1: expired premium + exhausted quota → error: "${BG_REMOVAL_LIMIT_REACHED}"`);
    assertEq((res._body as any)?.remaining, 0,
      'O1: expired premium + exhausted quota → remaining: 0');
    assertEq(fetchCallCount, 0,
      'O1: expired premium + exhausted quota → Photoroom fetch NOT called');

    delete overrides.mockPremium;
    delete overrides.mockPremiumExpiresAt;
    delete overrides.mockQuota;
    overrides.skipAuth   = true;
    overrides.testUserId = 'test-user-id';
    delete process.env.PHOTOROOM_API_KEY;
  ...
  // O2. Expired premium + quota still available → 200 with remaining field
  {
    const overrides = (require('../server/remove-background') as any)._testOverrides as {
      skipAuth: boolean;
      testUserId: string;
    ...
    overrides.skipAuth = true;
    overrides.testUserId = 'user-expired-entitlement-allowed';
    overrides.mockPremium = true;
    overrides.mockPremiumExpiresAt = new Date(Date.now() - 1_000).toISOString();
    overrides.mockQuota = {
      allowed:   true,
    ...
    assertEq(res._status, 200,
      'O2: expired premium + quota available → HTTP 200');
    assert(typeof (res._body as any).imageBase64 === 'string',
      'O2: expired premium + quota available → imageBase64 present');
    assertEq((res._body as any).remaining, expectedRemaining,
      `O2: expired premium + quota available → remaining=${expectedRemaining} (quota is applied)`);
    assert('remaining' in (res._body as any),
      'O2: expired premium → `remaining` field IS present (free-tier quota applies)');
  ...
  // O3. Active premium entitlement still bypasses quota.
  {
    const overrides = (require('../server/remove-background') as any)._testOverrides as {
      skipAuth: boolean;
      testUserId: string;
    ...
    overrides.skipAuth    = true;
    overrides.testUserId  = 'user-active-entitlement-premium';
    overrides.mockPremium = true;
    overrides.mockPremiumExpiresAt = new Date(Date.now() + 60_000).toISOString();
    overrides.mockQuota   = { allowed: false, count: 20, remaining: 0 };
    ...
    assertEq(res._status, 200,
      'O3: active premium entitlement + exhausted quota → HTTP 200 (quota bypassed)');
    assert(fetchCalled,
      'O3: active premium entitlement → Photoroom fetch called (quota gate skipped)');
    assert(!('remaining' in (res._body as any)),
      'O3: active premium entitlement → `remaining` field absent (no quota for premium users)');
